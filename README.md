...
## CLI
Brush can be used as a CLI. Run `cargo run -p brush-cli -- --help` to get an overview while developing. When packaging or distributing the tool, install the binary with `cargo install --path crates/brush-cli` to obtain the `brush-cli` executable. Every CLI command can work with `--with-viewer` which also opens the UI, for easy debugging.
